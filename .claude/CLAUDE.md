...
- `submission_contract.py` — `SubmissionContract`: a contract whose `tableschema` describes the bundle itself, plus `project_name` and an `extraction` block. The `tableschema` may declare neither `primaryKey` nor `foreignKeys` — nothing is stored under a bundle's name, so those belong to the contracts its targets name. Field constraints are unaffected.
