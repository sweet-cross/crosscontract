...
def _package_spec() -> CrossDataPackageReleaseSpec:
    return CrossDataPackageReleaseSpec.model_validate(
        {
            "name": "test-package",
            "title": "Test Package",
            "description": "A package for testing.",
            "resources": [
                {"data_instructions": {"fetch": {"contract": "my_resource"}}}
            ],
        }
    )
...
class TestSaveDataPackage:
    def test_creates_zip_with_expected_members(self, tmp_path, make_resources):
        mapping, _ = make_resources()
        out = tmp_path / "pkg.zip"

        save_data_package(_package_spec(), mapping, out)
    ...
    def test_data_file_matches(self, tmp_path, make_resources):
    ...
    def test_descriptor_roundtrips_as_frictionless(self, tmp_path, make_resources):
        mapping, _ = make_resources()
        out = tmp_path / "pkg.zip"
    ...
    def test_json_and_yaml_descriptors_match(self, tmp_path, make_resources):
        mapping, _ = make_resources()
        out = tmp_path / "pkg.zip"
    ...
    def test_no_suffix_appends_zip(self, tmp_path, make_resources):
        mapping, _ = make_resources()

        save_data_package(_package_spec(), mapping, tmp_path / "pkg")

        assert (tmp_path / "pkg.zip").exists()

    def test_wrong_suffix_raises(self, tmp_path, make_resources):
        mapping, _ = make_resources()

        with pytest.raises(ValueError, match="zip extension"):
            save_data_package(_package_spec(), mapping, tmp_path / "pkg.tar")

    def test_unsupported_format_raises(self, tmp_path, make_resources):
        mapping, _ = make_resources()
        ...
        with pytest.raises(ValueError, match="Unsupported data format"):
            save_data_package(_package_spec(), mapping, tmp_path / "pkg.zip")

    def test_parquet_resource_written(self, tmp_path, make_resources):
