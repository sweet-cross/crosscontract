...
- [X] `validate_submission(contract, df, check_existing_primary_key=True, check_existing_foreign_key=True, lazy=True) -> dict[str, pd.DataFrame]` on
  `CrossSubmitter`.
- [X] `contract` is typed `SubmissionContract` — the object, **not** `| str`. Name-based
  lookup is deliberately out of scope.
- [X] Runs in order, stopping at the first failure:
...
- [X] Returns step 3's result unchanged: validated frames keyed by **target name**.
- [X] All three flags are forwarded to **both** step 1 and step 3.
- [X] Defaults are `True` / `True` / `True` — deliberately opposite to the `False`
...
- [X] A `SubmissionHandler` is constructed **per call** on the raw `df` and not retained.
- [X] Step 1's coerced return value is **discarded**; extraction runs on the bundle as
  delivered. The docstring states this and why (`submit` uploads the original bundle
  and the platform re-runs extraction on it).
- [X] Docstring carries `Args:` / `Returns:` / `Raises:` listing all five exception types,
