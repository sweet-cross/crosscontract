...
- [ ] It records **why composites are the exception** — `IsValidPrimaryKey` names a meaning, because unpacking into one pandera check per sub-rule is what makes a failure legible, and because leaving the assembly to the caller is the copy-pasted derivation ADR 0005 exists to prevent. The coupling this reintroduces is paid for by one shared constructor per schema construct.
