...
- [ ] A row duplicated within the data is reported **once**, not twice, when existing primary keys are also supplied — the additional check replaces the standard one with the same name.
...
- **Modify:** `src/crosscontract/contracts/schema/schema.py`
  ...
  - Merge by `name`, additional wins:
    ```python
    merged = {c.name: c for c in standard}
    merged.update({c.name: c for c in additional})
    ```
...
- **The trap to avoid.** The additional check for a *self-referencing* foreign key must also carry `within=`. Build it through `IsSubsetOf.from_foreign_key(fk, allowed=...)`; constructing it by hand replaces the standard check with a weaker one and self-references silently stop being validated against the data's own rows.
...
- **Depends on:** `01-the-check-classes.md`.
