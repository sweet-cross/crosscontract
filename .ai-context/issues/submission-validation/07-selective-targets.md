...
- [X] `targets=None` validates every target, in declaration order.
- [X] A named subset validates only those targets, and the resolver is **not** asked to
  resolve the contracts of the others — asserted through the recording double.
- [X] `targets=[]` returns `{}` — empty means empty, `None` means all.
- [X] An unknown name in the list raises `KeyError` (inherited from `get_target`), not a
  silent skip.
- [X] A name repeated in the list is harmless: the returned dict collapses it. No dedupe
  guard, and the test must not pin whether the target was validated once or twice.
- [X] The docstring states the `None` vs `[]` distinction explicitly — it is the kind of
