...
## Acceptance Criteria
- [ ] `validate_target(target_name, contract=None, resolver=None, check_existing_primary_key=False, check_existing_foreign_key=False, lazy=True) -> pd.DataFrame`
      exists on `SubmissionHandler`.
- [ ] It composes `get_target_data(target_name)` with `contract.validate_data(...)` and
      returns the **validated, coerced** frame.
- [ ] `contract` takes precedence: when both are given, the resolver is never asked to
      resolve. Tested with a resolver whose contract *would* fail.
- [ ] When only `resolver` is given, the target's contract is resolved through it.
- [ ] All three pass-through parameters reach `validate_data` unchanged.
- [ ] With both `check_existing_*` flags `False`, the resolver receives **no `get_data`
      calls** — assert this explicitly; it is the "no unexpected network" property.
- [ ] A target claiming no rows returns an empty frame (the permanent handler-level
      counterpart to task 01).
- [ ] Coercion is asserted by a changed dtype, not merely by the absence of an exception.
- [ ] Google-style docstring covering `Args:` / `Returns:` / `Raises:`, including the note
