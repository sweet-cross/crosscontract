...
- [X] `src/tests/contracts/schema/validation/checks/test_granularity_checks.py` exists.
- [X] It carries the three-level parent map fixture from PRD §7:
  `{"ch": None, "de": None, "other": None, "ch_ag": "ch", "ch_other": "ch", "ch_ag_zurich": "ch_ag", "ch_ag_other": "ch_ag"}`.
- [X] Case 1 — aggregate and descendant in **one** group: `(A, 2030, ch)` and
  `(A, 2030, ch_ag)`. Asserts **per row** that the `ch` row fails and the `ch_ag` row
  passes. Asserting "something failed" is not sufficient.
- [X] Case 2 — aggregate and descendant in **different** groups: `(A, 2030, ch)` and
  `(B, 2030, ch_ag)`. Asserts every row passes. This is the core false-positive guard.
- [X] Case 3 — two **models** reporting at different granularities under the same
  ...
  is a group column.
- [X] The frame carries a `model` column and `group_columns` is `["model", "scenario", "year"]`, so every case above runs against a realistic multi-column group.
- [X] No other cases in this task.
- [X] The tests fail (import error is fine) until `02` lands.
...
- The check is constructed as
  `HasNoDescendantInGroup(label=..., column="region", group_columns=["scenario", "year"], parent_map=PARENT_MAP)` and called directly as `check(df)`, which returns a boolean
