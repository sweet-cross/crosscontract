...
- [X] `ContractResource.validate_dataframe` takes `check_dimension_granularity: bool = False` and passes it to `contract.validate_data`.
- [X] `add_data` does **not** set it — the check ships dormant and a caller opts in
  explicitly (PRD §5).
- [X] The docstring states that a `SchemaValidationError` from this check is converted to
  `ValidationError` like the others, and that the client-side result is advisory: the
  platform re-validates on ingest (ADR 0005).
- [X] A test asserts `add_data` does not enable the flag.
