...
- [ ] Two siblings in one group (`ch_ag` + `ch_other`) → all pass. An antichain is legal.
- [ ] Grandparent + grandchild with the parent absent (`ch` + `ch_ag_zurich`) → the
      grandparent fails. Proper ancestry is transitive.
- [ ] `ch` + `ch_other` → `ch` fails. The user-facing motivating example.
- [ ] Member absent from the parent map → passes ("unknown member" is `IsSubsetOf`'s
      defect).
- [ ] Null in the dimension column → passes, **asserted on the predicate's own return
      value** rather than on pandera behaviour.
- [ ] Null in a *group* column → rows sharing that null are grouped together and compared,
      not dropped.
- [ ] Empty-string parent treated as no parent (chain terminates).
- [ ] Cyclic parent map → terminates and does not hang.
- [ ] Empty DataFrame → passes.
- [ ] A group of one row → passes.
- [ ] Duplicate rows (same group, same member) → pass; a member is not its own proper
      ancestor, and `IsUnique` owns that defect.
- [ ] Multi-column group.
- [ ] `to_pandera()` returns exactly one check whose `error` equals `failure_message()`.
