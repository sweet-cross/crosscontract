...
## Acceptance Criteria
- [ ] `BaseContract.validate_data` takes `check_dimension_granularity: bool = False`.
- [ ] Set without a `resolver` → raises, reusing the existing "checking against existing
      values requires a resolver" message, extended to name the new flag.
- [ ] Unset → the resolver is not consulted for hierarchies at all, and
...
- [ ] The parent map is built from `resolver.get_data(dim_name, columns=[id_col,
      parent_col], unique=True)`, with `id_col` read off `fk.reference.fields` and
