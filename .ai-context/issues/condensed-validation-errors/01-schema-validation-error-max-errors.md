...
- [X] `SchemaValidationError.to_list(max_errors: int | None = None)` and
  `to_pandas(max_errors: int | None = None)` exist.
- [X] `max_errors=None` returns exactly today's output: no `count` key, one row per
  failure, same order.
- [X] With an integer, rows sharing `(schema_context, column, check, failure_case)`
...
- [X] With an integer, at most `max_errors` distinct `failure_case` values are kept per
...
- [X] An unhashable `failure_case` (a `list` from a `ListField` check) does not raise:
  the merge key uses `repr(failure_case)` for it, and the row keeps the original
  value.
- [X] Calling `to_list(max_errors=...)` does not mutate the cached `errors`: a
  following `to_list()` still returns the full report.
- [X] `to_pandas(max_errors)` returns `pd.DataFrame(self.to_list(max_errors))`.
- [X] Google-style docstrings (markdown only, `Args:` / `Returns:`) state that the
...
- Verified behaviour of the current report (scratch run, 2026-09-25):| Case                                                    | `schema_context`  | `failure_case`                     |
  | ------------------------------------------------------- | ------------------- | ------------------------------------ |
